...
const fakeApi = {
  // 模拟开始录音
  startRecording: () => new Promise(resolve => setTimeout(() => resolve(true), 500)),
  // 模拟停止录音
  stopRecording: () => new Promise(resolve => setTimeout(() => resolve({ audioUrl: 'audio_url.wav' }), 1000)),
  // 模拟处理语音并获取回复
  processAudio: (audioUrl) => new Promise(resolve => {
    setTimeout(() => {
      resolve({
        recognized: '你好，请问你能介绍一下自己吗？',
        response: '我是千问，一个18岁的女大学生，性格活泼开朗，喜欢交朋友。我对新事物充满好奇，尤其喜欢音乐和旅行。很高兴认识你！有什么我能帮你的吗？',
...
function RealTimeChat() {
  ...
  const handleStopRecording = async () => {
    ...
    try {
      ...
      // 停止录音获取音频
      const { audioUrl } = await fakeApi.stopRecording();
      
      // 处理音频
      const { recognized, response, audioResponse } = await fakeApi.processAudio(audioUrl);
