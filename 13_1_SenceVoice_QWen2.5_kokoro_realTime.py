...
def audio_recorder():
    ...
    stream = p.open(format=pyaudio.paInt16,
                    ...
                    input_buffer_size=CHUNK*10)  # 增加输入缓冲区大小
    ...
    while recording_active:
        try:
            data = stream.read(CHUNK, exception_on_overflow=False)  # 添加参数避免溢出报错
            ...
            print(f"音频录制错误: {e}")
            time.sleep(0.1)  # 发生错误时短暂暂停，避免过于频繁的错误
            continue
...
def save_audio_video():
    ...
    try:
        ...
        inference_thread.daemon = True  # 设置为守护线程，防止程序退出时阻塞
...
def play_audio(file_path):
    try:
        ...
        while pygame.mixer.music.get_busy():
            time.sleep(0.1)  # 减少CPU使用率，使用更短的等待时间
        print("播放完成！")
    except Exception as e:
        print(f"播放失败: {e}")
    finally:
...
if __name__ == "__main__":
    ...
    if platform.system() == 'Darwin':  # macOS
        print("检测到macOS系统，正在调整音频参数...")
        # 减小块大小可能有助于避免溢出
        CHUNK = 512
        print(f"已将CHUNK调整为{CHUNK}")
        
        # 检查可用设备
        p = pyaudio.PyAudio()
        info = p.get_host_api_info_by_index(0)
        num_devices = info.get('deviceCount')
        print(f"检测到{num_devices}个音频设备")
